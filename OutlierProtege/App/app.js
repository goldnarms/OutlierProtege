...
(function (App) {
    ...
    angular.module("App.services", ["ngResource"]).service("bandService", App.BandService.prototype.injection());
    ...
    //}
    app.config([
        "$sceDelegateProvider", "$routeProvider", "$locationProvider", "$provide", "$analyticsProvider", function ($sceDelegateProvider, $routeProvider, $locationProvider, $provide, $analyticsProvider) {
            ...
            $routeProvider.when("/", frontpageRoute).when("/Band/Create", {
    ...
    app.run([
        "breeze", "$rootScope", "$anchorScroll", "$routeParams", "common", "$window", "loginService",
        function (breeze, $rootScope, $anchorScroll, $routeParams, common, $window, loginService) {
            // After route change
            ...
            $rootScope.$on('$locationChangeStart', function (e, newUrl, oldUrl) {
                ...
                var shouldShow = _.any(showWhen, function (regexp) {
            ...
            var localStorageOutOfDate = userAuthorized && (!lastSet || moment(lastSet).isBefore(moment().subtract(moment.duration(1, "minutes"))));
            if (localStorageOutOfDate) {
                loginService.setLocalDataForCurrentUser(function () {
                }, function () {
    ...
    function handleBrowserQuirks($rootScope) {
        ...
        if (isIOS) {
            ...
            $rootScope.$on(App.BroadcastingEvents.Feed_Connected, function () {
                setTimeout(function () {
                    $(window).off("unload");
                }, 0);
            });
